...
bucket = s3.Bucket('miyamiyabucket')
...
while True:
    ...
    if target_object_count >= 1:
        ...
        if time.time() - last_send_time > MINIMUM_LINE_INTERVAL:
            ...
            # 通知メッセージリストからランダムに通知するメッセージを選択
            line_message = LINE_MESSAGE[random.randrange(len(LINE_MESSAGE))]
            ...
            # HTTPボディに APIで指定のmessageパラメタを指定して、通知メッセージをセット
            body = {"message": line_message}

            # ヘッダとデータ部（ボディ）を指定してLINE通知APIサーバにPOSTリクエストを送信
            res = requests.post(LINE_NOTIFY_API, headers=headers, data=body)
            ...
            # Transfer S3 Test
#            with open("/home/pi/UPLOAD.txt", mode ="a", "w") as f:
            with open("/home/pi/UPLOAD.txt", mode ="a", encoding='utf-8') as f:
                print(line_message, file=f)
            bucket.upload_file('/home/pi/UPLOAD.txt', 'UPLOAD.txt')
